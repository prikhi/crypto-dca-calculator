...
run (cleanArgs -> CleanedArgs{..}) = do
    let ladder = calculateLadderBuys LadderConfig
    ...
    putStrLn $ renderTable caFlipColumns
                           caPricePrecision
                           caAmountPrecision
                           ladder
...
argSpec =
    Args
            ...
            , argPricePrecision  =
                2 &= explicit &= name "p" &= name "price-precision" &= help
                    "Decimal places in the Price column (default: 2)"
            , argAmountPrecision =
                8 &= explicit &= name "a" &= name "amount-precision" &= help
